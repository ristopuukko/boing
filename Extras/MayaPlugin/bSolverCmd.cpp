...
MString bSolverCmd::typeName("bSolver");

bSolverCmd::bSolverCmd()
  : m_argDatabase(0),
    m_dgModifier(0)
{
}
...
MStatus
bSolverCmd::doIt(const MArgList &args)
{
    MStatus stat;
...
bSolverCmd::redoIt()
{
	...
	if (MFnDependencyNode(obj).typeId() == bSolverNode::typeId) {
	    return stat;
	}
    }
    ...
    m_dgModifier->renameNode(parentObj, "bSolverMain");
	m_dgModifier->doIt();

    MObject bSolverObj = m_dgModifier->createNode(bSolverNode::typeId, parentObj, &stat);
    ...
    m_dgModifier->renameNode(bSolverObj, "bSolver");
    m_dgModifier->doIt();

    // connect the time attribute
    MPlug plgInTime(bSolverObj, bSolverNode::ia_time);
    ...
    if (timeObj != MObject::kNullObj) {
		...
		fprintf(stderr, "ERROR : cannot find time node\n");
    }
    //force update of the solver on creation
    MPlug plgRigidBodies(bSolverObj, bSolverNode::oa_rigidBodies);
    bool update;
    plgRigidBodies.getValue(update);
    
    setResult(MFnDependencyNode(bSolverObj).name());
